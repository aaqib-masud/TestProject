...
public class Problem_Ufone_MW_Notification_10Minute_Alarms extends PTCL_Base_Problem {
	...
	static String urgency_impact = "20";
...
//        LOG.debug(String.format(" JAVA_LOG_RULE[%s]",log));
	...
	public void calculateProblemAlarmOtherAttribute(Group group, Action action) throws Exception {
		...
		//Defining Alarm Name
		alarm_name_target = "";
		if(additional_text_new.contains("LocationInfo=")) {
			String part_adt = additional_text_new.substring(additional_text_new.indexOf("LocationInfo=")+13);
			if(part_adt.contains("; "))
				alarm_name_target = part_adt.substring(0,part_adt.indexOf("; "));
			else
				alarm_name_target = part_adt.substring(0,30);
		}
